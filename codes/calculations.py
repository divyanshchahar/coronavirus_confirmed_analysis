...
# FUNCTION TO CALCULATE PERCENTAGE GROWTH RATE
# Input Parameters:
#   -   Dataframe with numbers of interest(confirmed cases/deaths/recovered) per day (grouped by country)
#   Output Parameters:
#   -   Dataframe with growth percentage of a day for each country, maximum, minimum growth percentage
def percent_calculator(df):
    countries = df['Country'].values.tolist() # extracting name of the countries
    dates = df.drop(['Country'], axis=1).columns.values.tolist() # extracting dates
    ...
    count = 0
    index_df_Final = [] # list to hold dates required after calculation is complete
    max_val = [] # list to hold maximum infection rate
    min_val = [] # list to hold minimun infection rate
    ...
    for i in range(0, length_countries): # first loop to select rows by country names
        ...
        for j in range(2, length_df_temp): # second loop to iterate through columns (going through dates of a country)
            try:
                k = round(((int(df_temp[j]) - int(df_temp[j-1])) / int(df_temp[j])) * 100, 3)
            except ZeroDivisionError:
                k = 0

            growthrate.append(k)

        max_val.append(max(growthrate)) # maximum infection rate
        min_val.append(min(growthrate)) # minimum infection rate
    ...
    df_final = pd.DataFrame(final_array, columns=index_df_Final)
    df_final.insert(0, 'Country', countries)
    df_final['max_infection_rate'] = max_val
    df_final['min_infect_rate'] = min_val
...
df_coronaC_newcases_percentgrowth = percent_calculator(df_coronaC_global) # Percentage Growth rate of new cases
df_coronaC_newcases_percentgrowth_cummulativeaverage = cummulativeaverage_calculator(df_coronaC_newcases_percentgrowth)
...
df_coronaC_newcases_percentgrowth.to_csv('coronavirus_newcases_percentgrowth.csv', index=False)
df_coronaC_newcases_percentgrowth_cummulativeaverage.to_csv('coronavirus_newcases_percentgrowth_cummulativeaverage.csv', index=False)
